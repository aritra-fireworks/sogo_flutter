...
import 'package:sogo_flutter/src/screens/member_privileges/member_privileges_details.dart';
...
class MemberPrivilegesView extends StatefulWidget {
  const MemberPrivilegesView({Key? key}) : super(key: key);

  @override
  State<MemberPrivilegesView> createState() => _MemberPrivilegesViewState();
}

class _MemberPrivilegesViewState extends State<MemberPrivilegesView> {
  ...
  void initState() {
    super.initState();
    promotionsManager.getMemberPrivileges(start: 0, offset: 0);
  }
      ...
      children: [
        ...
        Scaffold(
          ...
          appBar: AppBar(
            ...
            centerTitle: true,
            title: Text("Member Privileges", style: TextStyle(fontFamily: "GothamBold", fontSize: 16.sp, color: Colors.white),),
          ),
                  ...
                  switch (mallListSnapshot.data!.status) {
                    ...
                    case Status.COMPLETED:
                      ...
                      return StreamBuilder<ApiResponse<PromotionsListModel>?>(
                          stream: promotionsManager.memberPrivileges,
                            ...
                            if (eventsListSnapshot.hasData) {
                              switch (eventsListSnapshot.data!.status) {
                                ...
                                case Status.COMPLETED:
                                  ...
                                  return memberPrivilegesListBody(mallListSnapshot.data?.data?.malls, eventsListSnapshot.data?.data, context);
  ...
  Widget memberPrivilegesListBody(List<Mall>? malls, PromotionsListModel? promotions, BuildContext context) {
    return Container(
      decoration: const BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.vertical(top: Radius.circular(13)),
      ...
      child: NestedScrollView(
        ...
        body: ListView.builder(
          ...
          itemBuilder: (context, index) {
            return InkWell(
              onTap: (){
                Navigator.push(context, MaterialPageRoute(builder: (context) => MemberPrivilegesDetails(promotionId: promotions!.news![index].id!,),));
              },
