...
import 'package:sogo_flutter/src/widgets/event_card_view.dart';
...
class _EventsViewState extends State<EventsView> {
  ...
  void initState() {
    super.initState();
    eventsManager.getEventsList(start: 0, offset: 0);
  }
                ...
                if (mallListSnapshot.hasData) {
                  switch (mallListSnapshot.data!.status) {
                    ...
                    case Status.COMPLETED:
                      ...
                      return StreamBuilder<ApiResponse<EventsListModel>?>(
                          stream: eventsManager.eventsList,
                          builder: (BuildContext context, AsyncSnapshot<ApiResponse<EventsListModel>?> eventsListSnapshot) {
  ...
  Widget eventsListBody(List<Mall>? malls, EventsListModel? events, BuildContext context) {
    return Container(
      decoration: const BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.vertical(top: Radius.circular(13)),
      ...
      child: NestedScrollView(
        ...
        body: Builder(
          builder: (context) {
            if(events?.events != null && events!.events!.isNotEmpty){
              return ListView.builder(
                itemCount: events.events?.length??0,
                itemBuilder: (context, index) {
                  return InkWell(
                    onTap: (){
                      Navigator.push(context, MaterialPageRoute(builder: (context) => EventDetails(eventId: events.events![index].id!,),));
                    },
                    child: EventCardView(event: events.events![index], locationIcon: selectedMall!.mallIconWhite!,),
                  );
                },
              );
            } else {
              return Center(child: Text("No events found.", style: TextStyle(fontFamily: "GothamMedium", fontSize: 18.sp, color: Colors.grey),));
            }
          }
        ),
      ),
